...
const UpdateDeleteCalendar = ({show, onClose, event, onEvent, getCalendar, setEvent}) => {
    ...
    //event에 일정 내용을 담아 수정.
    const updateCalendarURL ="http://localhost:8080/calendar/updateCalendar"
    ...
    const onSubmit = () => {
        ...
        axios.post(updateCalendarURL,{...event, start: event.newStart, end: event.newEnd})
            .then(res => {
    ...
    const deleteCalendarURL ="http://localhost:8080/calendar/deleteCalendar"
    const onDelete = () => {
        const memberId = event.id
        console.log(memberId + "삭제하겠습니다")
        axios.post(deleteCalendarURL,null,{params:{memberId}})
            .then(res => {
                console.log(res.data)
                alert('삭제완')
                getCalendar()
        ...
        <div className="modal" onClick={onClose}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-body">
                    일정 수정 삭제<br/>
                    <p>
                        일정 시작일: <input type='type' value={formatDate(event.newStart)}/>
                        시간: <input type='time' onChange={onStartTime} value={startTime} name='eventStartTime'/>
                    </p>
                    <p>일정 마감일: <input type={'text'} value={formatDate(event.newEnd)} />
